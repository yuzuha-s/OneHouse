<!DOCTYPE html>
<html lang="ja">

<head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <link href="https://fonts.googleapis.com/css2?family=Noto+Sans+JP&display=swap" rel="stylesheet">

    @vite('resources/css/style.css')

    <title>@yield('title', 'OneHouse')</title>
</head>

<body class="fade">
    <header></header>

    <div class="section">
        <div class="nav">
            <div class="loginuser-nav">
                <ul>
                    <li>
                        <div class="icon2"><svg xmlns="http://www.w3.org/2000/svg" height="40px"
                                viewBox="0 -960 960 960" width="40px" fill="#fff">
                                <path
                                    d="M131-90.67v-524.11l349-262.55 349.67 262.22v524.44H567.33v-316H392.67v316H131Z" />
                            </svg></div>
                    </li>
                    <li><span class="title">{{ Auth::user()->name }}</span></li>
                    <li><a href="{{route('help-page')}}">
                            <div class="icon"><svg xmlns="http://www.w3.org/2000/svg" height="40px" viewBox="0 -960 960 960" width="40px" fill="#fff"><path d="M480.01-335.26q12.84 0 22.41-9.46 9.58-9.46 9.58-22.29 0-12.84-9.59-22.41-9.59-9.58-22.42-9.58-12.84 0-22.28 9.59-9.45 9.59-9.45 22.42 0 12.84 9.46 22.28 9.46 9.45 22.29 9.45Zm-21.29-117.07h42.82q.13-15.18 1.41-24.9 1.28-9.72 6.13-17.77 4.77-8.13 12.32-16.59 7.55-8.46 24.45-24.69 21.05-19.57 30.37-38.6 9.32-19.04 9.32-42.74 0-42.25-28.5-67.89t-73.78-25.64q-36.54 0-65.68 19.55-29.14 19.55-40.91 52.19l38.77 15.87q8.94-21.13 26.79-33.58 17.85-12.44 41.06-12.44 25.58 0 42.51 14.69 16.92 14.69 16.92 38.49 0 18.03-8.03 31.37-8.02 13.34-28.27 32.05-15.78 13.96-25.19 24.86-9.41 10.9-13.74 21.45-4.85 10.01-6.81 22.62-1.96 12.62-1.96 31.7Zm21.28 286q122.67-109.54 187.17-209.3 64.5-99.75 64.5-174.45 0-113.7-72.21-186.88-72.21-73.17-179.46-73.17-107.25 0-179.46 73.17-72.21 73.18-72.21 186.88 0 74.7 65.25 174.45 65.24 99.76 186.42 209.3Zm0 66.71Q329-230.46 253.54-343.15q-75.46-112.7-75.46-206.93 0-138.46 89.57-224.19Q357.23-860 480-860t212.35 85.73q89.57 85.73 89.57 224.19 0 94.23-75.46 206.93Q631-230.46 480-99.62Zm0-458.07Z"/></svg></div>
                        </a>

                    </li>
                </ul>
            </div>
            <div class="main-nav">
                <ul class="carousel">
                    <li class="phase svg-phase2"><a href="{{route('phase2')}}">
                            <div class="icon {{ request()->is('phase2') ? 'active' : '' }}"><svg
                                    xmlns="http://www.w3.org/2000/svg" height="40px" viewBox="0 -960 960 960"
                                    width="40px" fill="#fff">
                                    <path
                                        d="M692.87-615.85h61.03v-61.02h-61.03v61.02Zm0 165.29h61.03v-61.03h-61.03v61.03Zm0 164.87h61.03v-61.03h-61.03v61.03ZM640.92-140v-50.26h208.82v-579.48H460.51v99.1l-50.25-35.54V-820H900v680H640.92ZM60-140v-365.13l255.54-182.41 255.13 182.33V-140H367.28v-190.41H263.79V-140H60Zm50.26-50.26h103.28v-190.41h204v190.41h102.87v-289.33L315.54-625.03 110.26-479.24v288.98Zm530.66-352.56ZM417.54-190.26v-190.41h-204v190.41-190.41h204v190.41Z" />
                                </svg>

                            </div>
                            <span class="title">Maker</span>
                        </a>
                    </li>

                    <li class="phase svg-phase3"><a href="{{route('phase3.showLoanChart')}}">
                            <div class="icon {{ request()->is('phase3') ? 'active' : '' }}"><svg
                                    xmlns="http://www.w3.org/2000/svg" height="40px" viewBox="0 -960 960 960"
                                    width="40px" fill="#fff">
                                    <path
                                        d="M490.31-140v-.21.21q-51.59.46-112.77-7.42-61.18-7.89-113.94-25.18-52.75-17.3-88.18-44.66Q140-244.62 140-283.85q0 39.23 35.42 66.59 35.43 27.36 88.18 44.66 52.76 17.29 113.94 25.18 61.18 7.88 112.77 7.42Zm-10.18-208.31v-.54.54q-43.23-.15-83.67-4.01-40.43-3.86-76.96-11.98-36.52-8.11-69.15-20.16t-60.09-28.85q27.46 16.8 60.09 28.85t69.15 20.16q36.53 8.12 76.96 11.98 40.44 3.86 83.67 4.01ZM480-606.15q85.97 0 174.96-25.35 88.99-25.35 110.25-55.71-21.54-30.87-110.04-56.7-88.5-25.83-175.17-25.83-87.59 0-175.83 24.93-88.25 24.94-110.55 56.07 21.92 32.2 109.24 57.39 87.32 25.2 177.14 25.2Zm225.9 476.12h30.51v-167.38l74.31 74.56 21.54-21.54-111.23-111.23-111.24 111.23 21.54 21.54 74.57-74.56v167.38Zm15.13 62.34q-72.36 0-123.75-51.06-51.38-51.05-51.38-123.66 0-72.77 51.38-124.15 51.39-51.39 123.75-51.39 72.35 0 123.74 51.39 51.38 51.38 51.38 124.15 0 72.61-51.38 123.66-51.39 51.06-123.74 51.06ZM473.56-190.46q2.36 13.64 6.81 26.14 4.45 12.5 9.94 24.32-51.59.46-112.77-7.42-61.18-7.89-113.94-25.18-52.75-17.3-88.18-44.66Q140-244.62 140-283.85V-680q0-57.92 99.54-98.96Q339.08-820 480-820q140.92 0 240.46 41.04Q820-737.92 820-680v204.92q-11.82-5.48-24.32-9.55-12.5-4.06-25.94-6.42v-132.03q-53.84 31.54-130.19 48.87-76.34 17.34-159.96 17.21-85.77 0-162.06-17.46-76.3-17.46-127.27-48.62v156.49q50.56 33.15 127.55 50.59 76.98 17.44 161.78 17.44 11.31 0 22.1-.34 10.8-.33 22.11-1.15-9.67 11.87-17.39 24.78-7.72 12.91-13.97 26.96h-12.31q-87.26-.18-160.83-15.79-73.58-15.62-129.04-49.21v135.21q7.84 17.07 34.74 32.92 26.9 15.85 65.55 27.77 38.66 11.92 86.18 19.26 47.53 7.33 96.83 7.69Z" />
                                </svg></div><span class="title">Loan</span>
                        </a></li>

                    <li class="phase svg-phase4"><a href="{{route('phase4')}}">
                            <div class="icon {{ request()->is('phase4') ? 'active' : '' }}"><svg
                                    xmlns="http://www.w3.org/2000/svg" height="40px" viewBox="0 -960 960 960"
                                    width="40px" fill="#fff">
                                    <path
                                        d="M628.92-531.44v-1.51 1.51-158.25 158.25ZM185.95-163.8q-16.21 7.52-31.08-2.1Q140-175.51 140-193.8v-523.53q0-11.85 6.35-21.27 6.34-9.42 17.39-13.66l190.98-66.2 251.23 87.79 168.1-65.94q16.21-7.11 31.08 1.73Q820-786.05 820-768.15v327.53q-10.03-15.38-22.5-28.44-12.47-13.07-27.76-23.86v-249.9l-140.82 53.13v158.25q-13.43 1.21-25.8 3.35-12.38 2.14-24.45 6.14v-167.74l-197.34-67.8v517.41L185.95-163.8Zm4.31-54.05 140.82-53.64v-486l-140.82 47.23v492.41Zm454.2-4.38q38.59 0 64.71-25.73 26.11-25.73 26.32-65.3.2-38.59-26.12-64.8-26.32-26.22-64.91-26.22-38.59 0-64.81 26.22-26.21 26.21-26.21 64.8 0 38.85 26.21 64.94 26.22 26.09 64.81 26.09Zm0 50.25q-58.33 0-99.81-41.47-41.47-41.47-41.47-99.81 0-58.74 41.47-100.01 41.48-41.27 99.81-41.27 58.74 0 100.01 41.27t41.27 100.01q0 22.88-6.48 43.2-6.49 20.32-18.93 37.55L860-133.49 826.9-100l-99.41-99q-17.64 13.38-38.3 20.2-20.65 6.82-44.73 6.82ZM331.08-757.49v486-486Z" />
                                </svg></div><span class="title">Land size</span>
                        </a></li>

                    <li class="phase svg-phase1"><a href="{{route('phase1')}}">
                            <div class="icon {{ request()->is('phase1') ? 'active' : '' }}"><svg
                                    xmlns="http://www.w3.org/2000/svg" height="40px" viewBox="0 -960 960 960"
                                    width="40px" fill="#fff">
                                    <path
                                        d="M371.33-198.67v-66.66H840v66.66H371.33Zm0-248v-66.66H840v66.66H371.33Zm0-248.66V-762H840v66.67H371.33ZM192.67-160.67q-29.7 0-50.85-20.5-21.15-20.5-21.15-50.5t20.83-50.83q20.83-20.83 50.83-20.83t50.5 21.15q20.5 21.15 20.5 50.85 0 29.15-20.76 49.91-20.75 20.75-49.9 20.75Zm0-248q-29.7 0-50.85-20.95-21.15-20.96-21.15-50.38t21.15-50.38q21.15-20.95 50.85-20.95 29.15 0 49.9 20.95 20.76 20.96 20.76 50.38t-20.76 50.38q-20.75 20.95-49.9 20.95ZM192-657.33q-29.43 0-50.38-20.96-20.95-20.95-20.95-50.38 0-29.42 20.95-50.38Q162.57-800 192-800q29.42 0 50.38 20.95 20.95 20.96 20.95 50.38 0 29.43-20.95 50.38-20.96 20.96-50.38 20.96Z" />
                                </svg></div><span class="title">Checklist</span>
                        </a></li>

                </ul>
            </div>



            <div class="navigate">
                <nav>
                    <ul>
                        <li><a href="{{ route('user_setting') }}" class="nav-item">
                                <div class="icon2"><svg xmlns="http://www.w3.org/2000/svg" height="40px"
                                        viewBox="0 -960 960 960" width="40px" fill="#8C8C8C">
                                        <path
                                            d="m370-80-16-128q-13-5-24.5-12T307-235l-119 50L78-375l103-78q-1-7-1-13.5v-27q0-6.5 1-13.5L78-585l110-190 119 50q11-8 23-15t24-12l16-128h220l16 128q13 5 24.5 12t22.5 15l119-50 110 190-103 78q1 7 1 13.5v27q0 6.5-2 13.5l103 78-110 190-118-50q-11 8-23 15t-24 12L590-80H370Zm70-80h79l14-106q31-8 57.5-23.5T639-327l99 41 39-68-86-65q5-14 7-29.5t2-31.5q0-16-2-31.5t-7-29.5l86-65-39-68-99 42q-22-23-48.5-38.5T533-694l-13-106h-79l-14 106q-31 8-57.5 23.5T321-633l-99-41-39 68 86 64q-5 15-7 30t-2 32q0 16 2 31t7 30l-86 65 39 68 99-42q22 23 48.5 38.5T427-266l13 106Zm42-180q58 0 99-41t41-99q0-58-41-99t-99-41q-59 0-99.5 41T342-480q0 58 40.5 99t99.5 41Zm-2-140Z" />
                                    </svg></div><span class="title">Setting</span>
                            </a></li>
                        <form method="POST" action="{{ route('logout') }}">
                            @csrf
                            <li><button type="submit" class="logout-button" class="nav-item">
                                    <div class="icon2"><svg xmlns="http://www.w3.org/2000/svg" height="40px"
                                            viewBox="0 -960 960 960" width="40px" fill="#8C8C8C">
                                            <path
                                                d="M186.67-120q-27 0-46.84-19.83Q120-159.67 120-186.67v-586.66q0-27 19.83-46.84Q159.67-840 186.67-840h292.66v66.67H186.67v586.66h292.66V-120H186.67Zm470.66-176.67-47-48 102-102H360v-66.66h351l-102-102 47-48 184 184-182.67 182.66Z" />
                                        </svg></div><span class="title">Logout</span>
                                </button>
                            </li>
                        </form>
                    </ul>
                </nav>
            </div>
        </div>


        <main class="contant">
            <div class="wrapper">
                @yield('content')


                @yield('aside')

            </div>
        </main>
    </div>



    @vite('resources/js/fadein.js')


</body>

</html>
